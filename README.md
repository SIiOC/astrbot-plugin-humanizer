...
合并短时间内连续发送的私聊消息再交给 LLM，避免分段打字式刷屏（能力自独立插件 astrbot_plugin_chat_debounce 并入，仅私聊生效）。
...
让回复节奏跟随对话热度自动变化——真人连聊时秒回短句、隔了半天回复时不会热情过头。
...
跨消息的轻微情绪状态——被冷落会蔫一阵、被安抚会开心一阵，随对话逐条衰减，让语气有连续性而不是每条消息都从零开始。

- **三档情绪**：平静（neutral，默认）/ 有点小情绪（sulky）/ 心情不错（appy）。强度 0~1，低于阈值自动回到平静。
- **生气档可选**：打开「主动聊天 → 未回复小情绪」（默认关）后，主动消息连续未回复会让对话回复也带轻微小情绪（小委屈、轻轻嘟囔，绝不指责）；该开关同时管主动消息文案与对话侧情绪。
